...
<%
    ...
    try {
        Class.forName("com.mysql.jdbc.Driver"); // JDBCドライバをロードする
        Connection conn = DriverManager.getConnection(url, user, pass); // データベースに接続する
        PreparedStatement stmt = conn.prepareStatement("SELECT * FROM users WHERE username = ? AND password = ?"); // SQLクエリを用意する
